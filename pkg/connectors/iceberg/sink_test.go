...
func TestShouldUpdateIcebergTypeSkipsExistingWiderType(t *testing.T) {
	...
	if shouldUpdateIcebergType(iceberglib.PrimitiveTypes.Int64, iceberglib.DecimalTypeOf(20, 0), false) {
		t.Fatal("legacy long must be preserved because Iceberg cannot promote existing files to decimal(20,0)")
	}
	if shouldUpdateIcebergType(iceberglib.PrimitiveTypes.Int64, iceberglib.DecimalTypeOf(20, 0), true) {
		t.Fatal("legacy long must be preserved even when unsafe schema changes are enabled")
	}
...
func TestSyncSchemaPreservesLegacyLongForUnsignedBigint(t *testing.T) {
	current := iceberglib.NewSchema(1,
		iceberglib.NestedField{
			ID:       1,
			Name:     "id",
			Type:     iceberglib.PrimitiveTypes.Int64,
			Required: true,
		},
	)
	...
	if err != nil {
		t.Fatalf("new table metadata: %v", err)
	}
	tbl := table.New(
		[]string{"legacy_unsigned"},
		meta,
		"",
		nil,
		nil,
	...
	changed, err := syncSchema(
		updater,
		current,
		&model.TableSchema{
			SchemaName: "source",
			TableName:  "legacy_unsigned",
			Columns: []model.TableColumn{{
				Name:       "id",
				DataType:   "bigint",
	...
	if changed {
		t.Fatal("syncSchema reported a change for legacy long to decimal(20,0)")
	}
	if err := updater.Commit(); err != nil {
		t.Fatalf("commit unchanged updater: %v", err)
	}
}

func TestApplyDDLActionPreservesLegacyLongForUnsignedBigint(t *testing.T) {
	current := iceberglib.NewSchema(1,
		iceberglib.NestedField{
			ID:       1,
			Name:     "id",
	...
	tbl := table.New([]string{"legacy_ddl"}, meta, "", nil, nil)
	updater := tbl.NewTransaction().UpdateSchema(false, false)
	err = applyDDLAction(
		updater,
		icebergFieldTypesByName(current),
		ddlAction{
